...
const DeleteCourseLectureModal = ({ setShowDeleteLectureModal, deletedData }) => {
    ...
    const handleDeleteVideo = () => {
        dispatch(deleteLecture({ lectureId: deletedData?.lectureId, lectureName: deletedData?.lectureName, doc_type: deletedData?.doc_type }))
            .then(res => {
                ...
                if (res.meta.requestStatus === "fulfilled") {
                    ...
                    hotToast('Failed to delete lecture. Try again.', "error");
                }
            })
            .catch(error => {
                console.error("Error while submitting course:", error);
                getSweetAlert("Error", "Something went wrong while deleting the lecture.", "error");
            })
    };
    ...
    return (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-900 rounded-xl p-8 max-w-md w-full border border-gray-800">
                <h2 className="text-2xl font-bold mb-4">Delete Lecture?</h2>
                <p className="text-gray-400 mb-6">Are you sure you want to delete "{deletedData?.video_title}"? This action cannot be undone.</p>
...
export default DeleteCourseLectureModal